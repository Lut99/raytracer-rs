...
impl<T> Deref for BoundingBox<T> {
    type Target = T;

    #[inline]
    fn deref(&self) -> &Self::Target { &self.obj }
}
impl<T> DerefMut for BoundingBox<T> {
    #[inline]
...
#[derive(Clone, Debug)]
pub struct HitList {
    /// The [`Sphere`]s which use the [`StaticColour`] material.
    sphere_staticcolour : Vec<HitItem<Sphere<StaticColour>>>,
...
impl HitList {
    /// Provides immutable access to the internal list of statically coloured-spheres.
    #[inline]
    pub fn sphere_staticcolour(&self) -> &[HitItem<Sphere<StaticColour>>] { &self.sphere_staticcolour }
        ...
        for obj in value {
            ...
            match obj {
                ...
                Object::Sphere(s) => {
                    ...
                    match s.material {
                        Material::StaticColour(c) => {
                            result.sphere_staticcolour.push(HitItem::Object(BoundingBox::new(Sphere {
                                center : s.center,
                                radius : s.radius,
                ...
                Object::Group(g) => {
                    ...
                    // Compute a bounding box surrounding all of its statically coloured spheres (and we skip anything within a group)
                    let aabb: AABB = surround_list(ToplevelObjects{ list: &list.sphere_staticcolour, index: 0 });
                    // Then add it to the thing with a group prepended to it
                    result.sphere_staticcolour.reserve(1 + list.sphere_staticcolour.len());
                    result.sphere_staticcolour.push(HitItem::Group(BoundingBox{ obj: list.sphere_staticcolour.len(), aabb }));
                    result.sphere_staticcolour.extend(&list.sphere_staticcolour);
